...
Page* BufferPoolManager::fetch_page(PageId page_id) {
    ...
    if (it != page_table_.end()) {
        // 1.1 目标页有被page_table_记录，将其所在frame固定(pin)，并返回目标页
    ...
    // 3. 从page_table中移除victim page的映射（如果存在）
    if (victim_page->get_page_id().page_no != INVALID_PAGE_ID) {
...
bool BufferPoolManager::unpin_page(PageId page_id, bool is_dirty) {
 ...
 std::scoped_lock lock{latch_};
    ...
    // 1. 尝试在page_table_中搜寻page_id对应的页
    auto it = page_table_.find(page_id);
    if (it == page_table_.end()) {
        // 1.1 P在页表中不存在，返回false
        return false;
    }
    
    // 1.2 P在页表中存在，获取对应的frame_id和page
    frame_id_t frame_id = it->second;
    Page* page = &pages_[frame_id];
    ...
    // 3. 根据参数is_dirty，更改P的is_dirty_
    // 注意：只有当is_dirty为true时才设置为dirty，避免意外清除dirty标志
...
Page* BufferPoolManager::new_page(PageId* page_id) {
   ...
   std::scoped_lock lock{latch_};
    ...
    // 4. 清理page并设置新的page_id
    page->reset_memory();
    
    // 从page_table中移除旧页面的映射（如果存在）
...
void BufferPoolManager::flush_all_pages(int fd) {
    ...
    for (int i = 0; i < pool_size_; i++) {
        ...
        if (page_id.page_no != INVALID_PAGE_ID && page_id.fd == fd) {
            // 将页面写回磁盘
            disk_manager_->write_page(fd, page_id.page_no, page->get_data(), PAGE_SIZE);
            // 清除脏页标志
            page->is_dirty_ = false;
        }
    }
