...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    // Todo:
    // 1.lseek()定位到文件头，通过(fd,page_no)可以定位指定页面及其在磁盘文件中的偏移量.相当于是移动文件指针
    // 2.调用write()函数
    // 注意write返回值与num_bytes不等时 throw InternalError("DiskManager::write_page Error");
    off_t off_set =   static_cast<off_t>(page_no * PAGE_SIZE);    //page_no * PAGE_SIZE计算指定⻚面在文件中的偏移量
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    off_t off_set =   static_cast<off_t>(page_no*PAGE_SIZE);
    if(lseek(fd,off_set,SEEK_SET) == -1){
        throw UnixError();
    }
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    if(is_file(path)){
        ...
        if (unlink(path.c_str()) == -1) {   //delete a name and possibly the file it refers to
            throw UnixError();
        }
    }
}
...
int DiskManager::read_log(char *log_data, int size, int offset) {
    // read log file from the previous end,log_fd_ is the log file descriptor
    if (log_fd_ == -1) {
    ...
    if(size == 0) return 0;
    lseek(log_fd_, offset, SEEK_SET);                    //从offset位置开始读
    ssize_t bytes_read = read(log_fd_, log_data, size);  //读全部或者就读出一部分
